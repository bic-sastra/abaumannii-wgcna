...
# FIXED: Module preview, Added paper info
# =====================================================
  ...
  dashboardBody(
    ...
    tabItems(
      ...
      tabItem(tabName = "modules",
        ...
        fluidRow(
          box(title = "Module Statistics", width = 6,
              tableOutput("module_stats")),
          box(title = "Module Network Preview", width = 6,
              visNetworkOutput("module_preview", height = "400px"))
        )
      ),
      ...
      tabItem(tabName = "help",
        fluidRow(
          box(title = "About This Application", width = 12, status = "primary",
              ...
              h4("🔗 Links"),
              tags$ul(
                tags$li(a("GitHub Repository", href = "https://github.com/yourusername/abaumannii-wgcna", target = "_blank")),
...
server <- function(input, output, session) {
  ...
  # =============================================
  # FIXED: Module preview network with better handling
  # =============================================
  output$module_preview <- renderVisNetwork({
    req(input$module_select != "All")
    
    # Check if module exists in network edges
    if(!(input$module_select %in% names(data$network_edges))) {
      # Return a message network
    ...
    edges <- data$network_edges[[input$module_select]] %>%
      filter(correlation >= input$cor_threshold)
    
    # If no edges after filtering
    if(nrow(edges) == 0) {
      # Return a message network
    ...
    # Normal network rendering
    nodes <- unique(c(edges$gene1, edges$gene2))
  ...
  output$gene_network <- renderVisNetwork({
    ...
    req(length(input$gene_search) > 1)
    ...
    if(nrow(relevant_edges) > 0) {
      ...
      nodes <- data.frame(id = "no_edges", 
                          ...
                          title = "These genes are not directly connected",
                          color = "#95A5A6",
                          size = 30)
      edges <- data.frame(from = character(0), to = character(0))
  ...
  output$full_network <- renderVisNetwork({
    ...
    if(input$module_select == "All") {
      ...
      req(input$module_select %in% names(data$network_edges))
